...
Same shape and semantics as oncura-programs/core/store.py — the portal's
audit log persists in the repo so it survives Cloud restarts. Only the
GITHUB_REPO constant changes between apps.
"""
...
GITHUB_REPO = os.environ.get("ONCURA_APPS_REPO", "alexanderjordain/oncura-apps")
GITHUB_BRANCH = os.environ.get("ONCURA_APPS_BRANCH", "main")
...
def load_json(rel_path: str, default=None):
    """Load data/<rel_path>. Returns (data, sha). sha is the GitHub blob SHA
    needed to commit an update, or None when data came from the local file.

    Token present → GitHub is the writable source of truth. No token → local
    file is, so local edits don't get shadowed by the read-only GitHub copy.
    """
    if _github_token():
...
def save_json(rel_path: str, data, message: str, sha: str | None = None, retries: int = 3):
    ...
    Returns (ok, info). ok=False when no token configured or retries exhausted.
    """
    ...
    for _ in range(retries):
        ...
        try:
            ...
            if r.status_code in (409, 412, 422):
                # Concurrent edit — refetch sha and retry. The portal's access log
                # is append-only and rarely contested, so a simple retry suffices.
                sha = None
                continue
